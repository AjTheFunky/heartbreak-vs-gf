...
class PlayState extends MusicBeatState
{
	...
	var upperBoppers2:FlxSprite;
	var upperBoppers3:FlxSprite;
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'bopanties':
				dialogue = CoolUtil.coolTextFile(Paths.txt('bopanties/bopantiesDialogue'));
			case 'highly-fresh':
				dialogue = CoolUtil.coolTextFile(Paths.txt('highly-fresh/highly-freshDialogue'));
			case 'gfilfw':
				dialogue = CoolUtil.coolTextFile(Paths.txt('gfilfw/gfilfwDialogue'));
			case 'heartaches':
				dialogue = CoolUtil.coolTextFile(Paths.txt('heartaches/heartachesDialogue'));
			case 'apart':
				dialogue = CoolUtil.coolTextFile(Paths.txt('apart/apartDialogue'));
			case 'crash-n-burn':
				dialogue = CoolUtil.coolTextFile(Paths.txt('crash-n-burn/crash-n-burnDialogue'));
			case 'holiday':
				dialogue = CoolUtil.coolTextFile(Paths.txt('holiday/holidayDialogue'));
			case 'mistletoe':
				dialogue = CoolUtil.coolTextFile(Paths.txt('mistletoe/mistletoeDialogue'));
			case 'date-night':
				dialogue = CoolUtil.coolTextFile(Paths.txt('date-night/date-nightDialogue'));
			case 'senpai':
		...
		switch(SONG.song.toLowerCase())
		{
			...
			case 'pico' | 'blammed' | 'philly'| 'date-night-og': 
					{
					curStage = 'philly';
					...
					add(bg);
						...
						var city:FlxSprite = new FlxSprite(-150).loadGraphic(Paths.image('philly/city'));
					...
					var streetBehind:FlxSprite = new FlxSprite(-150, 50).loadGraphic(Paths.image('philly/behindTrain'));
					streetBehind.setGraphicSize(Std.int(streetBehind.width * 1.4));
					add(streetBehind);
					...
					var street:FlxSprite = new FlxSprite(-150, streetBehind.y).loadGraphic(Paths.image('philly/street'));
			...
			case 'cocoa' | 'eggnog' | 'holiday':
			{
					...
					upperBoppers = new FlxSprite(-240, -100);
			...
			case 'mistletoe':
			{
					...
					upperBoppers2.scrollFactor.set(0.33, 0.33);
					upperBoppers2.setGraphicSize(Std.int(upperBoppers2.width * 0.85));
					upperBoppers2.updateHitbox();
					add(upperBoppers2);
			...
			case 'date-night':
			{
					...
					upperBoppers3 = new FlxSprite(-240, -140);
					upperBoppers3.frames = Paths.getSparrowAtlas('christmas/upperBop3');
					upperBoppers3.animation.addByPrefix('bop', "Upper Crowd Bob3", 24, false);
					upperBoppers3.antialiasing = true;
					upperBoppers3.scrollFactor.set(0.33, 0.33);
					upperBoppers3.setGraphicSize(Std.int(upperBoppers3.width * 0.85));
					upperBoppers3.updateHitbox();
					add(upperBoppers3);
	...
	function transformation(?dialogueBox:DialogueBox):Void
	{
		var black:FlxSprite = new FlxSprite(-100, -100).makeGraphic(FlxG.width * 2, FlxG.height * 2, FlxColor.BLACK);
		black.scrollFactor.set();
		add(black);
		...
		var gfEvil:FlxSprite = new FlxSprite();
	    gfEvil.frames = Paths.getSparrowAtlas('weeb/gfCrazy');
     	gfEvil.animation.addByPrefix('idle', 'Animation', 24, false);
		// gfEvil.setGraphicSize(Std.int(senpaiEvil.width * 6));
		// gfEvil.scrollFactor.set();
		// gfEvil.updateHitbox();
	    gfEvil.screenCenter();

		if (SONG.song.toLowerCase() == 'apart' || SONG.song.toLowerCase() == 'crash-n-burn')
		{
			remove(black);
					...
					if (SONG.song.toLowerCase() == 'crash-n-burn')
					{
						...
						new FlxTimer().start(0.3, function(swagTimer:FlxTimer)
						{
							...
							else
							{
								...
								FlxG.sound.play(Paths.sound('transform'), 1, false, null, true, function()
								{
									remove(gfEvil);
									remove(red);
									FlxG.camera.fade(FlxColor.WHITE, 0.01, true, function()
	...
	*/
	...
	override function beatHit()
	{
		...
		if (curSong.toLowerCase() == 'gfilfw' && curBeat >= 168 && curBeat < 200 && camZooming && FlxG.camera.zoom < 1.35)
		{
		...
		if (curBeat % 8 == 7 && curSong == 'Bopanties')
		{
			boyfriend.playAnim('hey', true);
		}

		if (curBeat % 16 == 15 && SONG.song == 'Tutorial' && dad.curCharacter == 'girlfriend' && curBeat > 16 && curBeat < 48)
			{
				boyfriend.playAnim('hey', true);
			}
		...
		switch (curStage)
		{
			...
			case 'mall2':
				upperBoppers2.animation.play('bop', true);
				bottomBoppers.animation.play('bop', true);
				santa.animation.play('idle', true);
			case 'mall3':
				upperBoppers3.animation.play('bop', true);
				bottomBoppers.animation.play('bop', true);
				santa.animation.play('idle', true);
